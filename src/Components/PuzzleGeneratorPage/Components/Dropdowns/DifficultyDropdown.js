...
function DifficultyDropdown({ topic, difficultyCallback, selected, selectedCallback }) {
    ...
    return (
        <Form.Select value={selected} onClick={(e) => {difficultyCallback(e.target.value); selectedCallback(e.target.value);}} onChange={(e) => {selectedCallback(e.target.value); difficultyCallback(e.target.value)}}>
